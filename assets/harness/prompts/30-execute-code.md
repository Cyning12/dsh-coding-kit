...
- 归档（active→done）**只能**走 `npx dsh-coding-kit task close --file <task> --yes`：`CLOSE: PASS` 后方可归档；机械校验 **invoke hats 集合** / 自检结论 / 验收勾选 / slug / 状态 / R1 review / graph_delta / KPI / experience / **wiki_delta**（及晋升指针），任一不过 **不执行** mv（v2.2+ · hats 集合 v2.12+ · wiki v2.18+）——上述机械校验 dsh-coding-kit@1.2.x **未接线**（接线计划见 PRD_DEF-003 阶段二）；当前以 task 文档人工核对为准
