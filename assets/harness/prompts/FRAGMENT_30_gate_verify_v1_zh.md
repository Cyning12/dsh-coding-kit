...
pre-30 invoke：`required ∩ {10,20,00}` 文件是否齐全？ {是/否}（**v2.14+**：`verify --task` **硬闸** · 缺失即 `VERIFY: BLOCKED · missing pre-30 invoke hats` · `may_start_30=false`；缺 40 **不挡** 30；`minimal` 无 preRequired 不挡；`--allow-invoke-gap` 豁免留痕）
